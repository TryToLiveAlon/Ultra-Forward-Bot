...
async def pub_(bot, message):
    ...
    if not _bot:
        return await msg_edit(m, "You Didn't Add Any Bot. Please Add A Bot Using /settings !", wait=True)
    try:
    ...
    if locked:
        try:
            ...
            return await stop(client, user)
        temp.IS_FRWD_CHAT.remove(sts.TO)
        await send(client, user, "<b>🎉 ғᴏʀᴡᴀʀᴅɪɴɢ ᴄᴏᴍᴘʟᴇᴛᴇᴅ 🥀 <a href=https://t.me/TryToLiveAlon>SUPPORT</a>🥀</b>")
...
async def edit(msg, title, status, sts):
    ...
    text = TEXT.format(i.fetched, total_files_left, 4567544,i.duplicate, i.deleted, i.skip, status, percentage, estimated_total_time)
